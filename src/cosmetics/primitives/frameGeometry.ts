 ...
 *
 * Kept out of the component and free of anything that has to be mounted, because the arithmetic here
 * is the part worth testing: an anchor whose signs run the wrong way puts somebody's ornament on the
 * far side of their card; a nine-slice whose widths and outsets are swapped puts the band inside the
 * card instead of around it. None of that is a type error and none of it is visible in a diff.
 *
 ...
 * created the row but not finished uploading to it. The part then draws nothing rather than a broken
 * image.
 */
 ...
 *
 * Rounded to a hundredth rather than to a pixel: at a third of size the difference between 11.33 and
 * 11 is a band that misses the card's edge, and the browser draws fractions perfectly well.
 */
