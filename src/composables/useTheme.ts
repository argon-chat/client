...
export function useTheme() {
  ...
  const applyTheme = (themeId?: ThemeId) => {
    ...
    logger.warn("applyTheme called, current theme: ", theme);
    ...
    const nativeTheme =
      theme === "dark" ? "Dark" : theme === "light" ? "White" : "OLED";
  ...
  const applyAppearanceSettings = () => {
    ...
    const fontFamily = persistedValue<string>(
    ...
    const uiDensity = persistedValue<string>(
      "appearance.uiDensity",
      "comfortable"
    );
    const borderRadius = persistedValue<number>(
      "appearance.borderRadius",
      0.75
    );
    const accentColor = persistedValue<string>(
      "appearance.accentColor",
      "blue"
    );
    const enableAnimations = persistedValue<boolean>(
      "appearance.enableAnimations",
      true
    );
    const reduceMotion = persistedValue<boolean>(
    ...
    const timestampFormat = persistedValue<string>(
      "appearance.timestampFormat",
      "24h"
    );
    const highContrast = persistedValue<boolean>(
      "appearance.highContrast",
      false
    );
    const dyslexiaFont = persistedValue<boolean>(
      "appearance.dyslexiaFont",
      false
    );
    const colorBlindMode = persistedValue<string>(
      "appearance.colorBlindMode",
      "none"
    );

    // Apply font (or dyslexia font)
    const selectedFont = dyslexiaFont.value
