<template>
  <div class="profile-header-uploader relative group w-full h-32 rounded-lg overflow-hidden">
...
const getHeaderUrl = (fileId: string) => {
  return `https://koko.argon.gl/api/v1/download/${fileId}`;
};
...
const onHeaderChange = async (event: Event) => {
  ...
  if (isAnimated) {
    try {
      ...
      const blobId = await uploadProfileHeader(file);
