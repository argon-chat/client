...
export const usePreference = defineStore("preference", () => {
  ...
  const isEnable_playSoftEnterSound = persistedValue<boolean>(
    "sound_active_playSoftEnterSound",
    true,
  );
  const isEnable_playReconnectSound = persistedValue<boolean>(
    "sound_active_playReconnectSound",
    true,
  );
  const isEnable_playSoftLeaveSound = persistedValue<boolean>(
    "sound_active_playSoftLeaveSound",
    true,
  );
  const isEnable_playMuteAllSound = persistedValue<boolean>(
    "sound_active_playMuteAllSound",
    true,
  );
  const isEnable_playUnmuteAllSound = persistedValue<boolean>(
    "sound_active_playUnmuteAllSound",
    true,
  );
  const isEnable_playNotificationSound = persistedValue<boolean>(
    "sound_active_playNotificationSound",
    true,
  );
  const isEnable_playRingSound = persistedValue<boolean>(
    "sound_active_playRingSound",
    true,
  );
